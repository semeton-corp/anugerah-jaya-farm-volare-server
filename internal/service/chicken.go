...
// Note :
// 1. Mid-period restocking / movement.If chickens are moved into the cage during the 30-day window (`MoveChickenCage`), the "earliest" total doesn't represent the population that was actually exposed to mortality risk
...
func (s *ChickenService) calculateChickenMonitoringMortalityRateLast30Days(chickenCageId uint64, upsertMonitoring *entity.ChickenMonitoring) (float64, error) {
	...
	chickenMonitorings, err := s.repository.GetChickenMonitorings(&dto.GetChickenMonitoringFilter{
		ChickenCageId: chickenCageId,
		StartDate:     param.DateParam(last30Days),
		EndDate:       param.DateParam(today),
	})
	if err != nil {
		return 0, err
	}
	...
	earliestAliveChicken := uint64(0)
	earliestMonitoringAt := time.Time{}
	isUpsertApplied := false

	for _, chickenMonitoring := range chickenMonitorings {
		computedMonitoring := chickenMonitoring
		if upsertMonitoring != nil && upsertMonitoring.Id > 0 && chickenMonitoring.Id == upsertMonitoring.Id {
			computedMonitoring.TotalChicken = upsertMonitoring.TotalChicken
			computedMonitoring.TotalDeathChicken = upsertMonitoring.TotalDeathChicken
			isUpsertApplied = true
		}

		totalDeathChicken += computedMonitoring.TotalDeathChicken
		if earliestMonitoringAt.IsZero() || earliestMonitoringAt.After(computedMonitoring.CreatedAt) {
			earliestMonitoringAt = computedMonitoring.CreatedAt
			earliestAliveChicken = computedMonitoring.TotalChicken
		}
	}

	if upsertMonitoring != nil && (upsertMonitoring.Id == 0 || !isUpsertApplied) {
		totalDeathChicken += upsertMonitoring.TotalDeathChicken
		upsertMonitoringCreatedAt := upsertMonitoring.CreatedAt
		if upsertMonitoringCreatedAt.IsZero() {
			upsertMonitoringCreatedAt = time.Now()
		}

		if earliestMonitoringAt.IsZero() || earliestMonitoringAt.After(upsertMonitoringCreatedAt) {
			earliestAliveChicken = upsertMonitoring.TotalChicken
		}
	}

	if earliestAliveChicken == 0 {
		return 0, nil
	}
...
func (s *ChickenService) CreateChickenMonitoring(request dto.CreateChickenMonitoringRequest, userId uuid.UUID) (dto.ChickenMonitoringResponse, error) {
	...
	if err := validateChickenMonitoringTotals(chickenCage.TotalChicken, request.TotalSickChicken, request.TotalDeathChicken); err != nil {
		return dto.ChickenMonitoringResponse{}, err
	}
	...
	chickenMonitoring.MortalityRate, err = s.calculateChickenMonitoringMortalityRateLast30Days(request.ChickenCageId, &chickenMonitoring)
	if err != nil {
		s.log.Error("failed to calculate mortality rate by last 30 days", zap.Error(err))
		return dto.ChickenMonitoringResponse{}, err
	}
	...
	if chickenMonitoring.MortalityRate < 0.5 {
		notificationJsonParsed, err := json.Marshal(entity.Notification{
			...
			Description:          fmt.Sprintf(constant.ChickenStatusNotification, chickenMonitoring.ChickenCage.Cage.Name, chickenMonitoring.MortalityRate),
		})
		if err != nil {
...
func (s *ChickenService) UpdateChickenMonitoring(id uint64, request dto.UpdateChickenMonitoringRequest, userId uuid.UUID) (dto.ChickenMonitoringResponse, error) {
	...
	if chickenMonitoring.ChickenCageId != request.ChickenCageId {
		return dto.ChickenMonitoringResponse{}, errx.BadRequest("chicken cage cannot be changed when updating monitoring")
	}
	...
	previousTotalFeed := chickenMonitoring.TotalFeed
	currentChicken := totalChickenBaseline - request.TotalDeathChicken
	chickenMonitoring.TotalChicken = totalChickenBaseline
...
func (c *ChickenService) DeleteChickenMonitoring(id uint64, userId uuid.UUID) error {
	...
	chickenMonitorings, err := c.repository.GetChickenMonitorings(&dto.GetChickenMonitoringFilter{
		ChickenCageId: chickenMonitoring.ChickenCageId,
	})
	if err != nil {
		c.log.Error("failed get chicken monitorings by chicken cage id", zap.Error(err))
		return err
	}

	if len(chickenMonitorings) > 0 && chickenMonitorings[0].Id != id {
		return errx.BadRequest("only latest chicken monitoring can be deleted")
	}

	if chickenMonitoring.TotalDeathChicken >= chickenMonitoring.TotalChicken {
		return errx.BadRequest("cannot delete monitoring that emptied chicken cage")
	}
...
func (c *ChickenService) GetChickenOverview(filter dto.GetChickenOverviewFilter) (dto.ChickenOverviewResponse, error) {
	...
	for _, cc := range chickenCages {
		...
		case enum.ChickenCategoryDOC:
			totalDOCChicken += cc.TotalChicken
		case enum.ChickenCategoryGrower:
			totalGrowerChicken += cc.TotalChicken
		case enum.ChickenCategoryPreLayer:
			totalPreLayerChicken += cc.TotalChicken
		case enum.ChickenCategoryLayer:
			totalLayerChicken += cc.TotalChicken
		case enum.ChickenCategoryAfkir:
			totalAfkirChicken += cc.TotalChicken
		}
	}
...
func (c *ChickenService) GetKPIScoreChickenPerWeek(locationId uint64, month enum.Month, year uint64) (map[int]float64, error) {
	...
	totalLiveChickenInWeek := make(map[int]uint64)
	...
	for _, key := range keys {
		mortalityRate := 0.0
		if monitoringCountInWeek[key] > 0 {
			mortalityRate = totalMortalityRateInWeek[key] / float64(monitoringCountInWeek[key])
		}

		hdpRate := 0.0
		if totalLiveChickenInWeek[key] > 0 {
...
func (s *ChickenService) CreateChickenHealthMonitoring(request dto.CreateChickenHealthMonitoringRequest, userId uuid.UUID) (dto.ChickenHealthMonitoringResponse, error) {
	...
	if chickenHealthMonitoringType == enum.ChickenHealthItemTypeVaccineRoutine {
		latestChickenAgeVaccineRoutine := int64(chickenCage.ChickenAge)
		_, err = s.cageService.UpdateChickenCage(chickenCage.Id, dto.UpdateChickenCageRequest{
			TotalChicken:                   chickenCage.TotalChicken,
			LatestChickenAgeVaccineRoutine: &latestChickenAgeVaccineRoutine,
...
func (s *ChickenService) ArrivalConfirmationChickenProcurement(id uint64, request dto.ArrivalConfirmationChickenProcurementRequest, userId uuid.UUID) (dto.ChickenProcurementResponse, error) {
	...
	if chickenProcurement.IsArrived {
		return dto.ChickenProcurementResponse{}, errx.BadRequest("chicken procurement has already arrived")
	}

	if request.Quantity > chickenProcurement.Cage.Capacity {
		return dto.ChickenProcurementResponse{}, errx.BadRequest("receive quantity is greater than cage capacity")
	}
...
func (s *ChickenService) CreateAfkirChickenSale(request dto.CreateAfkirChickenSaleRequest, userId uuid.UUID) (dto.AfkirChickenSaleResponse, error) {
	...
	if chickenCage.TotalChicken < request.TotalSellChicken {
		return dto.AfkirChickenSaleResponse{}, errx.BadRequest("total sell chicken must be less than or equal total chicken")
	}
...
func (s *ChickenService) CreateAfkirChickenSalePayment(afkirChickenSaleId uint64, request dto.CreateAfkirChickenSalePaymentRequest, userId uuid.UUID) (dto.AfkirChickenSaleResponse, error) {
	...
	paymentStatus, paidDate, err := procurementPaymentStatus(totalCurrentPayment.Add(nominal), afkirChickenSale.TotalPrice, false)
	if err != nil {
		return dto.AfkirChickenSaleResponse{}, err
	}
...
func (s *ChickenService) GetChickenPerformances(filter dto.GetChickenPerformanceFilter) ([]dto.ChickenPerformanceResponse, error) {
	...
	if filter.Date.Value().Year() == today.Year() &&
		...
		eggMonitoringMap := make(map[uint64]dto.EggMonitoringResponse)
		eggMonitorings, err := s.eggService.GetEggMonitoringDetails(dto.GetEggMonitoringFilter{
			Date:       param.DateParam(today),
			LocationId: filter.LocationId,
			CageId:     filter.CageId,
		})
		...
		for _, eggMonitoring := range eggMonitorings {
			eggMonitoringMap[eggMonitoring.ChickenCage.Id] = eggMonitoring
		}
		...
		for _, chickenCage := range chickenCages {
			...
			totalEgg := (eggMonitoringMap[chickenCage.Id].TotalKarpetCrackedEgg * constant.TotalEggPerKarpet) +
				eggMonitoringMap[chickenCage.Id].TotalRemainingCrackedEgg +
				(eggMonitoringMap[chickenCage.Id].TotalKarpetGoodEgg * constant.TotalEggPerKarpet) +
				eggMonitoringMap[chickenCage.Id].TotalRemainingGoodEgg
			totalGoodEgg := (eggMonitoringMap[chickenCage.Id].TotalKarpetGoodEgg * constant.TotalEggPerKarpet) + eggMonitoringMap[chickenCage.Id].TotalRemainingGoodEgg
			...
			response.MortalityRate = chickenMonitoringMap[chickenCage.Id].MortalityRate * 100.0

			if chickenCage.TotalChicken > 0 {
				response.HDP = (float64(totalGoodEgg) / float64(chickenCage.TotalChicken)) * 100.0
			}
...
func (s *ChickenService) GetChickenAndWarehouseOverview(filter dto.GetChickenAndWarehouseOverviewFilter) (dto.ChickenAndWarehouseOverviewResponse, error) {
	...
	var totalFeed, totalWeightGoodEgg float64
	var totalEgg, totalDeathChicken, totalChicken, totalGoodEgg uint64
	var totalMortalityRate float64
	var totalMortalityMonitoringCount uint64
	...
	if err != nil {
		s.log.Error("failed get egg monitorings")
		return dto.ChickenAndWarehouseOverviewResponse{}, err
	}
	for _, eggMonitoring := range eggMonitorings {
		eggMonitoringMap[eggMonitoring.ChickenCage.Id] = eggMonitoring
	...
	for _, chickenCage := range chickenCages {
		chickenMonitoring, ok := chickenMonitoringMap[chickenCage.Id]
		eggMonitoring, hasEggMonitoring := eggMonitoringMap[chickenCage.Id]
		hasChickenMonitoring := ok

		if hasChickenMonitoring && hasEggMonitoring {
			totalChicken += chickenCage.TotalChicken
			totalFeed += chickenMonitoring.TotalFeed
			totalDeathChicken += chickenMonitoring.TotalDeathChicken
			totalMortalityRate += chickenMonitoring.MortalityRate
			totalMortalityMonitoringCount += 1
			totalEgg += (eggMonitoring.TotalKarpetGoodEgg * constant.TotalEggPerKarpet) + eggMonitoring.TotalRemainingGoodEgg
			totalWeightGoodEgg += eggMonitoring.TotalWeightGoodEgg
			totalGoodEgg += (eggMonitoring.TotalKarpetGoodEgg * constant.TotalEggPerKarpet) + eggMonitoring.TotalRemainingGoodEgg
		}
	...
	if totalEgg > 0 {
		avgGoodEggWeight = (totalWeightGoodEgg / float64(totalEgg)) * 1000.0
	}

	avgMortality := 0.0
	if totalMortalityMonitoringCount > 0 {
		avgMortality = totalMortalityRate / float64(totalMortalityMonitoringCount)
	}

	avgFCR := 0.0
	if totalWeightGoodEgg > 0 {
		avgFCR = totalFeed / float64(totalWeightGoodEgg)
	}

	avgHDP := 0.0
	if totalChicken > 0 {
		avgHDP = float64(totalGoodEgg) / float64(totalChicken)
	}
	...
	return dto.ChickenAndWarehouseOverviewResponse{
		ChickenPerformanceSummary: dto.ChickenPerformanceSummaryResponse{
			...
			AverageEggWeight:     avgGoodEggWeight,
			AverageMortalityRate: avgMortality * 100.0,
			AverageFCR:           avgFCR,
			AverageHDP:           avgHDP * 100.0,
		},
...
func (s *ChickenService) GetChickenAndCompanyOverview(filter dto.GetChickenAndCompanyOverviewFilter) (dto.ChickenAndCompanyOverviewResponse, error) {
	...
	today := time.Date(time.Now().Year(), time.Now().Month(), time.Now().Day(), 0, 0, 0, 0, time.Local)
	var totalFeed, totalWeightGoodEgg float64
	var totalGoodEgg, totalDeathChicken, totalChicken uint64
	var totalMortalityRate float64
	var totalMortalityMonitoringCount uint64
	var totalDOCChicken, totalGrowerChicken, totalPreLayerChicken, totalLayerChicken, totalAfkirChicken uint64
	chickenCages, err := s.cageService.GetChickenCages(dto.GetChickenCageFilter{
	...
	avgEggWeight := 0.0
	if totalGoodEgg != 0 {
		avgEggWeight = (totalWeightGoodEgg / float64(totalGoodEgg)) * 1000.0
	}
